	...
	class Model
	{
		...
		private function loadField()
		{
			...
			while($row = mysqli_fetch_assoc($result)){
				...
				$this->field[] = $row['Field'];//获得表的所有字段 并赋值给$field
		...
		public function insert($data = array())
		{
			...
			foreach($data as $k=>$v){
				...
				if(in_array($k,$this->field)){//判断字段有效性
					$fieldList[] = $k;
					$valueList[] = $v;
				}
			}
		...
		$sql = "insert into {$this->tabname}(".implode(",",$fieldList).")
		...
		{
			if(empty($data)){//如果insert方法里面 没有给参数 默认使用$_POST传递的参数
				$data = $_POST;
			}
			//print_r($data);
